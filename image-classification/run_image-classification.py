...
MODEL_INPUT_SIZE = {"pixel_values": (1, 3, 224, 224)}
...
def benchmark(pipeline, image, nb_pass):
    elapsed_times = []
    forward_times = []
...
if __name__ == "__main__":
    ...
    if compare_outputs:
        _, _, eager_outputs = benchmark(classifier, image, 1)
    ...
    if compare_outputs:
        _, _, optimized_outputs = benchmark(classifier, image, 1)
    ...
    with ContextManagers(inference_context):
        elapsed_times, forward_times, output = benchmark(classifier, image, warm_up_steps + run_steps)
